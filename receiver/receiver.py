...
class Receiver(nn.Module):
    ...
    def forward(
        ...
        if self.mode == 'image_proj_norm':
            ...
            pass
        # Future: Decompression module (trainable) will be inserted here:
        #   vision_features = self.decompression_module(received_vision_embedding)  # Trainable, no no_grad()
        ...
        # Mode 1: Process from vision_tower to image_proj_norm
            ...
            # Note: vision_features is now outside no_grad() to allow gradient flow if trainable module follows
        else:
        ...
        # Text embedding generation is FROZEN - use no_grad() for memory efficiency
        with torch.no_grad():
        ...
        # Merge vision and text embeddings using Florence-2's method (FROZEN - use no_grad())
        # Note: Merge operation is frozen, but vision_features should allow gradient flow if trainable module precedes
        with torch.no_grad():
